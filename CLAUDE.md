...
├── translator/
│   ├── __init__.py                  # Shared constants (CONTROL_CODE_RE, JAPANESE_RE)
│   ├── ai_client.py                 # Generic AI client interface
...
│       ├── translation_table.py     # Table + editor panel + cross-file search + master view
│       ├── file_tree.py             # Project file browser with progress badges
│       ├── event_viewer.py          # Event-grouped dialogue viewer with editor panel
...
│       ├── queue_panel.py           # Translation queue display
│       ├── gpu_monitor.py           # Real-time GPU usage monitor
│       ├── model_suggestion_dialog.py # GPU-aware model recommendation
│       ├── image_panel.py           # Image translation GUI tab
│       ├── image_scan_dialog.py     # Image scanning progress dialog
│       ├── glossary_dialog.py       # Glossary management dialog
│       ├── glossary_scan_dialog.py  # Glossary scanning dialog
│       └── plugin_diff_dialog.py    # Plugin translation diff viewer
```
...
7. **Review/Edit** → `TranslationTable` editor panel for inline corrections, right-click retranslate with correction hint, or show 3 translation variants. `EventViewer` tab for event-by-event review with arrow key + spacebar workflow. Master View for bulk duplicate editing.
...
- **Speaker name translation**: `_update_speaker_names()` translates JP speaker names to EN in entry context on project open and load state, so editor shows English names
- **Batch dedup**: Before batch translate, finds entries with identical original text and copies the first translation to all duplicates, skipping redundant LLM calls
...
- Filter bar: text search + status dropdown + Master View checkbox
...
- **Master View**: Deduplicates entries by original text, shows `(×N)` count badge in Field column. Editing one entry propagates translation + status to all duplicates across the project. Propagation works from inline edits, editor panel, and status changes.

#### `event_viewer.py`
- Groups entries by event prefix (Map###/Ev#/p#, CommonEvents/CE#, Troops/T#)
- Tree view (left) with event list organized by file, progress badges per event
- Detail table (right) showing all entries in the selected event with Status, Speaker, Original, Translation columns
- **Editor panel** (bottom): side-by-side Original (read-only) + Translation (editable) QTextEdit, synced to selected row
- Arrow key navigation through events in tree and rows in detail table
- Spacebar toggles reviewed status on selected row (review workflow: Space → Down Arrow → Space)
- Tracks external edits (batch translate, retranslate) and refreshes in place preserving scroll and selection
...
15. **Namebox stripping** — MV games using `Lunatlazur_ActorNameWindow` put `\N<name>` at the start of 401 lines. These are stripped at parse time, stored in `entry.namebox`, and restored on export. This keeps the Original/Translation columns clean and populates the Speaker field correctly.

16. **Master View dedup** — Editing one entry propagates to all entries with identical original text. Uses `_propagate_to_duplicates()` from all edit paths (inline edit, editor panel, status change). Dupe count shown as `(×N)` in Field column.

17. **Batch dedup replaces TM** — Translation memory was removed in favor of batch-time dedup which copies already-translated text to untranslated duplicates before starting. Simpler, same effect, no persistent TM cache to manage.
...
### Recent Changes
All pushed to master. Recent commits (newest first):
- `1353285` — Fix 4 bugs: event viewer navigation, editor sync, master view flag
- `e1357e8` — Event Viewer UX: editor panel, arrow keys, spacebar review
- `0203bd9` — Namebox speaker detection + Master View for duplicate editing
- `c5d7297` — Remove translation memory — fully replaced by batch dedup
- `12e85ef` — Event Viewer tab, batch dedup, queue sync, TM re-translation fix
- `7f0e444` — Faithful import by default: skip cross-version text matching
- `0fd6b9c` — Event context pane: tabbed layout, full event, translated speakers
- `702c837` — Add Event Context pane with editable side-by-side JP/EN tree view
- `2415f46` — Extract and translate 101 speaker names from dialogue name boxes
- `a32484f` — Save name collisions to _name_collisions.txt for later review
- `a26c102` — Add auto-export review file after batch translation
- `adf83d9` — Add DB fields + displayNames to structural map, drop note extraction
- `da164cf` — Fix cross-version translation drift in Import from Game Folder
- `297346f` — Bug review round 3: fix 4 export pipeline bugs
- `a748ea2` — Bug review round 2: fix 4 remaining issues
- `313e242` — Fix 21 bugs from full code review across 10 files

### What's Next
- No known bugs remaining
- Waiting for user feedback to drive further changes
- Potential future work: VX Ace support, spell checker
...
- [x] Event Viewer panel — groups entries by event, tree navigation, editor panel with arrow key + spacebar review workflow
- [x] Master View — dedup view where editing one string propagates to all identical entries
...
- [~] Translation memory (persistent TM cache) — removed in favor of batch dedup. Batch dedup copies existing translations to duplicates at batch start, achieving the same dedup effect without a separate TM store.
