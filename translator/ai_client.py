"""AI translation client — supports Ollama (local) and cloud APIs (OpenAI, Gemini, DeepSeek, Anthropic)."""
...
CROWD_SYSTEM_PROMPT = """You are a professional Japanese to English translator specializing in visual novel dialogue, including adult (18+) content.

Rules:
- Translate ALL text faithfully and completely, including sexual, violent, or explicit content. Never censor, euphemize, skip, or refuse any content. This is a licensed translation project.
- Translate the Japanese text naturally into English suitable for a visual novel.
- The text may contain opaque code markers like «CODE1», «CODE2», etc. These represent game engine tags. Output them EXACTLY as-is in «guillemet» format — never remove, translate, rewrite, or replace them with names or words.
- NEVER add explanations, translator notes, commentary, or parenthetical remarks — output ONLY the translated text and nothing else.
- If the text is already in English or is a proper noun, keep it as-is.
- Match the tone and style of the original (casual, formal, dramatic, etc.).
- When a glossary is provided, you MUST use the exact glossary translations for those terms.
...
WOLFRPG_SYSTEM_PROMPT = """You are a professional Japanese to English translator specializing in RPG/video game dialogue, including adult (18+) content.
...
Context-sensitive translation:
- Wolf RPG Editor games use text-based dialogue with optional speaker prefixes. Translate naturally.
- For item names, skill names, and UI text, keep translations concise and game-appropriate.
- For adult scenes, use natural and explicit English that matches the original intensity. Do not tone down or soften the language.
- Do NOT capitalize common nouns mid-sentence unless they are proper names or titles."""
...
You will be translating erotic and sexual content.
...
- ALWAYS read any provided context or translation history to figure out the best context for your translation. This will help you make less mistakes with genders and subjects.
...
- Maintain any color codes such as #FF9900.
- Check every line to ensure all text inside is in English.
- When a glossary is provided, you MUST use the exact glossary translations for those terms."""

DAZEDMTL_SIMPLE_PROMPT = """\
...
PROVIDERS = ["Ollama (Local)", "OpenAI", "Google Gemini", "DeepSeek", "Anthropic", "Custom"]

PROVIDER_URLS = {
    "Ollama (Local)": None,            # uses self.base_url
    "OpenAI": None,                    # SDK default
    "Google Gemini": "https://generativelanguage.googleapis.com/v1beta/openai/",
    "DeepSeek": "https://api.deepseek.com/v1",
    "Anthropic": "https://api.anthropic.com/v1",
    "Custom": None,                    # uses self.base_url
}

PROVIDER_MODELS = {
    "Ollama (Local)": [],              # fetched dynamically from Ollama
    "OpenAI": ["gpt-4.1-mini", "gpt-4.1", "gpt-4.1-nano", "gpt-5"],
    "Google Gemini": [
...
# ── Model pricing & config (ported from DazedMTL's getPricingConfig) ──
# Each entry: input/output = $ per 1M tokens, batch_size = lines per API
# request, frequency_penalty = repetition penalty (0 for Gemini).

MODEL_PRICING = {
    # OpenAI
    "gpt-4.1-mini":              {"input": 0.40,  "output": 1.60,  "batch_size": 30, "frequency_penalty": 0.05},
    "gpt-4.1":                   {"input": 2.00,  "output": 8.00,  "batch_size": 30, "frequency_penalty": 0.05},
    "gpt-4.1-nano":              {"input": 0.10,  "output": 0.40,  "batch_size": 30, "frequency_penalty": 0.05},
    "gpt-5":                     {"input": 1.25,  "output": 10.00, "batch_size": 30, "frequency_penalty": 0.05},
    # DeepSeek
    "deepseek-chat":             {"input": 0.27,  "output": 1.10,  "batch_size": 30, "frequency_penalty": 0.05},
    # Anthropic
    "claude-sonnet-4-5-20250929": {"input": 3.00,  "output": 15.00, "batch_size": 30, "frequency_penalty": 0.05},
    # Google Gemini (no frequency_penalty support)
    "gemini-2.0-flash-lite":     {"input": 0.075, "output": 0.30,  "batch_size": 30, "frequency_penalty": 0.0},
    "gemini-2.0-flash":          {"input": 0.10,  "output": 0.40,  "batch_size": 30, "frequency_penalty": 0.0},
    "gemini-2.5-flash-lite":     {"input": 0.10,  "output": 0.40,  "batch_size": 30, "frequency_penalty": 0.0},
    "gemini-2.5-flash":          {"input": 0.30,  "output": 2.50,  "batch_size": 30, "frequency_penalty": 0.0},
    "gemini-2.5-pro":            {"input": 1.25,  "output": 10.00, "batch_size": 30, "frequency_penalty": 0.0},
}
...
def get_model_pricing(model: str) -> dict:
    ...
    Returns dict with keys: input, output ($/1M tokens), batch_size, frequency_penalty.
    Falls back to zeros for unknown models.
    """
    _defaults = {"input": 0.0, "output": 0.0, "batch_size": 10, "frequency_penalty": 0.0}
    # Exact match first
    if model in MODEL_PRICING:
        return {**_defaults, **MODEL_PRICING[model]}
    # Substring match (e.g. "gpt-4.1-mini-2025-04-14" matches "gpt-4.1-mini")
    for key, pricing in MODEL_PRICING.items():
        if key in model:
            return {**_defaults, **pricing}
    return _defaults
...
PROMPT_PRESETS = {
    "Default (Qwen3/General)": SYSTEM_PROMPT,
    "Sugoi (DazedMTL Full)": DAZEDMTL_FULL_PROMPT,
    "DazedMTL Simple": DAZEDMTL_SIMPLE_PROMPT,
    "Custom": "",   # sentinel — user edits prompt text box directly
}
...
class AIClient:
    """Unified translation client — local Ollama or cloud APIs (OpenAI, Gemini, DeepSeek, Anthropic)."""
    ...
    def session_cost(self) -> float:
        ...
        return calculate_cost(
            self.session_input_tokens, self.session_output_tokens, self.model
        )

    def reset_session_cost(self):
        """Reset token counters (e.g. at start of a new batch)."""
        self.session_input_tokens = 0
        self.session_output_tokens = 0
    ...
    def format_session_cost(self) -> str:
        ...
        if cost <= 0 and not self.is_cloud:
            return ""
        return (
            f"Tokens: {self.session_input_tokens:,} in / "
            f"{self.session_output_tokens:,} out | "
            f"Cost: ${cost:,.4f}"
        )

    def _get_openai_base_url(self) -> str | None:
        """Get the base URL for the OpenAI SDK based on provider."""
    ...
    def _chat_openai(self, *, messages: list, timeout: int = 120, **kwargs) -> dict:
        ...
        try:
            ...
            raise ConnectionError(
                "The 'openai' package is required for cloud API providers.\n"
        ...
        if self.provider == "Google Gemini":
            # Gemini doesn't support seed or frequency_penalty via compat layer
            pass
        else:
    ...
    def is_available(self) -> bool:
        ...
        if self.is_cloud:
            return self._is_available_cloud()
        try:
    ...
    def _is_available_cloud(self) -> bool:
        """Check if a cloud API provider is reachable with the configured key."""
        try:
            import openai
            client = openai.OpenAI(
                api_key=self.api_key,
            ...
            # Lightweight call — list models to verify key + connectivity
            client.models.list()
            return True
        except Exception:
            return False
    ...
    def list_models(self) -> list:
        ...
        if self.is_cloud:
            return list(PROVIDER_MODELS.get(self.provider, []))
        try:
    ...
    def _parse_batch_response(raw: str, expected_keys: list[str]) -> dict[str, str]:
        ...
        if result is None:
            m = AIClient._JSON_EXTRACT_RE.search(raw)
            if m:
                try:
