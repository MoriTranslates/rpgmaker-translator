...
- **Python 3.10+**
- **NVIDIA GPU** (or CPU-only with reduced speed)
...
After opening a project, a dialog will appear showing all actors found in the game. You can assign genders (male/female/non-binary) so the translator uses correct pronouns. The tool auto-detects genders from profile text but you should verify.

### Translating

- **Batch Translate**: Click the **Batch Translate** button to translate all untranslated entries. Progress and ETA are shown in the status bar.
...
- **Save State**: Saves all translations to a JSON file so you can close and resume later
- **Load State**: Loads a previously saved state file to continue work
- **Auto-Save**: The tool auto-saves every 2 minutes if you have entries loaded

### Exporting to the Game

- **Export**: Writes all translated text back into the game's original JSON files in the `data/` folder
- **Export TXT**: Saves a human-readable text patch file for reference

### Word Wrap

Click **Apply Word Wrap** to automatically format translated text to fit the game's message window width. The tool detects message plugins (YEP_MessageCore, VisuMZ_MessageCore, etc.) and adjusts line lengths accordingly.
